...
function DoctorSignupScreen() {
  ...
  // Initial form values for doctor - aligned with backend models
  const initialValues = {
    email: "",
    full_name: "",
    phone_number: "",
  ...
  const loadFormData = async () => {
    try {
      const savedData = await AsyncStorage.getItem('doctorSignupFormData');
      if (savedData) {
  ...
  const saveFormData = async (formData: any) => {
    try {
      await AsyncStorage.setItem('doctorSignupFormData', JSON.stringify({
        formData,
        timestamp: Date.now()
      }));
    } catch (error) {
  ...
  const clearFormData = async () => {
    try {
      await AsyncStorage.removeItem('doctorSignupFormData');
    } catch (error) {
...
const styles = StyleSheet.create({
  ...
  topSection: {
    flex: 0.4, // Increased significantly to give more space for the full image
  },
  backgroundImage: {
    flex: 1,
    width: '100%',
